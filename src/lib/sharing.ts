...
const SHARE_LINK_TTL_MS = Number(process.env.NEXT_PUBLIC_SHARE_TTL_MS) || 7 * 24 * 60 * 60 * 1000; // 7 days default
...
export async function getSharedGame(id: string): Promise<GameData | null> {
  ...
  try {
    ...
    if (snap.exists()) {
      const data = snap.data() as GameData & { expiresAt?: string };
      if (data.expiresAt && new Date(data.expiresAt).getTime() < Date.now()) {
        console.warn('[Sharing] Shared link has expired');
        return null;
      }
